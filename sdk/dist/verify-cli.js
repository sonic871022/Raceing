...
import { parseReplayJsonl, recheckReplayArtifact, } from './engine/index.js';
function usage() {
 ...
 */
export async function runVerifyCli(argv, io = {}) {
    ...
    try {
        ...
        const imported = await import(pathToFileURL(resolve(cwd, adapterPath)).href);
        const resolver = imported.resolveReplayReducer ?? imported.default;
        if (typeof resolver !== 'function') {
            throw new TypeError('adapter must export resolveReplayReducer(context) or a default resolver');
        }
        ...
        if (json) {
            ...
            stdout(`${report.verdict} · ${report.format} ${report.formatVersion}`
            ...
            for (const seat of report.signatures.seats) {
                stdout(`seat ${seat.seat}: ${seat.validSignatures} valid signatures, `
