import { assertJsonValue, canonicalJson, } from '../protocol.js';
import { advanceTick, replayMetricsFor, } from './contracts.js';
import { COMMITMENT_SCHEME, createCommitmentHash, } from './commitment.js';
import { createDmath } from './dmath.js';
import { locationKey } from './locations.js';
import { SUBMISSION_SIGNATURE_SCHEME, periodicSignaturePreimageV1, signatureBytesFromBase64, submissionChainHashV1, submissionGenesisHashV1, submissionPreimageV1, submissionRosterHashV1, verifyEd25519Base64, } from './submission-signatures.js';
import { recheckTranscript, runLevelSeed, } from './replay.js';
...
function projectRecordActions(stream) {
    const projected = [];
...
function rejectUnknownProperties(value, allowed, label, problems) {
    const allowedKeys = new Set(allowed);
    for (const key of Object.keys(value)) {
        if (!allowedKeys.has(key))
            problems.push(`${label} has unknown property ${key}`);
    }
}
...
function validateV12IntegrityFields(candidate, label, problems) {
    ...
    if (chainPresent.length === chain.length) {
        ...
        try {
            signatureBytesFromBase64(candidate['prevChainHash'], `${label} prevChainHash`, 32);
        }
        catch (error) {
    ...
    if (candidate['sig'] !== undefined) {
        try {
            signatureBytesFromBase64(candidate['sig'], `${label} sig`, 64);
        }
        catch (error) {
...
function validatePermutation(value) {
    return Array.isArray(value)
        && value.every((entry) => Number.isSafeInteger(entry)
            && entry >= 0 && entry < value.length)
        && new Set(value).size === value.length;
}
function replayTotals(levels) {
    return {
        totalStars: levels.reduce((sum, level) => sum + (level.result.status === 'won' ? (level.result.stars ?? 0) : 0), 0),
...
export function createReplayArtifact(input) {
    ...
    if (problems.length > 0)
        throw new ReplayFormatError(problems);
    return artifact;
}
/** Lift an existing single-level SDK transcript into the portable envelope. */
export function transcriptToReplayArtifact(header, actions, options) {
 ...
 */
export function validateReplayArtifact(value) {
    const problems = [];
    if (!isRecord(value))
    ...
    if (header['kind'] !== 'header')
        problems.push('header.kind must be header');
    ...
    if (header['formatVersion'] !== GAOS_REPLAY_FORMAT_VERSION
        ...
        problems.push(`header.formatVersion must be ${GAOS_REPLAY_LEGACY_FORMAT_VERSION}`
    ...
    if (!validU32(header['seed']))
        problems.push('header.seed must be an unsigned 32-bit integer');
    ...
    for (const field of ['signaturePolicy', 'timeoutPolicy', 'extensions']) {
    ...
    if (hasV12IntegritySemantics(header['formatVersion'])) {
        ...
        if (hasPolicy) {
            ...
            if (!isRecord(policy)) {
                ...
                if (policy['scheme'] !== SUBMISSION_SIGNATURE_SCHEME) {
                    problems.push(`header.signaturePolicy.scheme must be ${SUBMISSION_SIGNATURE_SCHEME}`);
                }
            }
        }
        if (hasRoster) {
            if (!Array.isArray(header['seatKeys']) || header['seatKeys'].length === 0) {
                ...
                try {
                    submissionRosterHashV1(header['seatKeys']);
                }
                catch (error) {
                    problems.push(`header.seatKeys is invalid: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
        ...
        if (timeoutPolicy !== undefined) {
            if (!isRecord(timeoutPolicy)) {
                ...
                rejectUnknownProperties(timeoutPolicy, ['mode', 'windowTicks'], 'header.timeoutPolicy', problems);
    ...
    if (!Array.isArray(levels) || levels.length === 0) {
        ...
        for (const [index, candidate] of levels.entries()) {
            ...
            else if (header['seedPolicy'] === GAOS_REPLAY_DERIVED_SEEDS
            ...
            if (!Object.hasOwn(candidate, 'level'))
                problems.push(`level ${index} must include level data`);
            ...
            if (!isRecord(result)) {
                ...
                rejectUnknownProperties(result, ['status', 'stars', 'actionsUsed', 'extensions'], `level ${index} result`, problems);
    ...
    if (!isRecord(totals)) {
        ...
        rejectUnknownProperties(totals, ['totalStars', 'totalActionsUsed', 'extensions'], 'header.totals', problems);
    ...
    if (!Array.isArray(actions)) {
        ...
        const levelTicks = new Map();
        const permutation = validatePermutation(header['perm']) ? header['perm'] : [];
        for (const [index, action] of actions.entries()) {
            ...
            if (action['kind'] !== 'action')
                problems.push(`action at index ${index} kind must be action`);
            if (!Number.isSafeInteger(action['n'])
                || sequenceBase === undefined
                || action['n'] !== sequenceBase + index) {
                problems.push(`action at index ${index} has non-contiguous sequence number ${String(action['n'])}`);
            }
            ...
            const parseActionId = (field) => {
                ...
                if (!Number.isSafeInteger(number) || number < 1 || number > permutation.length) {
                    problems.push(`action ${String(action['n'])} ${field} must be within Action 1..${permutation.length}`);
                    return undefined;
                }
                return number - 1;
            };
            ...
            if (wire !== undefined && canonical !== undefined && permutation[wire] !== canonical) {
                problems.push(`action ${String(action['n'])}: wire ${String(action['wireId'])} `
            ...
            for (const field of ['hostTime', 'clientTime']) {
                if (action[field] !== undefined && !validNonNegativeInteger(action[field])) {
                    problems.push(`action ${String(action['n'])} ${field} must be a non-negative safe integer`);
                }
            }
            if (hasV12IntegritySemantics(header['formatVersion'])) {
            ...
            if (action['targets'] !== undefined) {
                if (!Array.isArray(action['targets'])) {
                    ...
                    for (const [targetIndex, target] of action['targets'].entries()) {
                        try {
                            if (isRecord(target)) {
                                rejectUnknownProperties(target, ['container', 'coord'], `action ${String(action['n'])} target ${targetIndex}`, problems);
                            }
                            locationKey(target);
                        }
                        catch {
            ...
            if (header['formatVersion'] === GAOS_REPLAY_LEGACY_FORMAT_VERSION
                ...
                problems.push(`action ${String(action['n'])} v1.1 fields require formatVersion 1.1`);
            }
            if (action['commit'] !== undefined) {
                ...
                if (!isRecord(commit)) {
                    ...
                    rejectUnknownProperties(commit, ['commitmentId', 'scheme', 'hash'], `action ${String(action['n'])} commit`, problems);
            ...
            if (action['reveal'] !== undefined) {
                ...
                if (!isRecord(reveal)) {
                    ...
                    rejectUnknownProperties(reveal, ['commitmentId', 'salt', 'payload'], `action ${String(action['n'])} reveal`, problems);
    ...
    if (records !== undefined) {
        if (!isGroupedReplayVersion(header['formatVersion'])) {
            problems.push(`records require header.formatVersion ${GAOS_REPLAY_UNSIGNED_FORMAT_VERSION}`
                + `, ${GAOS_REPLAY_SIGNED_FORMAT_VERSION}, or ${GAOS_REPLAY_FORMAT_VERSION}`);
        }
        if (!Array.isArray(records)) {
            ...
            let previousLevelIndex = -1;
            const recordTicks = new Map();
            const permutation = validatePermutation(header['perm']) ? header['perm'] : [];
            const validateResolutionInput = (candidate, label, actionRecord = false) => {
                if (!isRecord(candidate)) {
                ...
                for (const field of ['wireId', 'canonicalId']) {
                ...
                if (isRecord(commit)) {
                    rejectUnknownProperties(commit, ['commitmentId', 'scheme', 'hash'], `${label} commit`, problems);
                }
                if (candidate['targets'] !== undefined) {
                    if (!Array.isArray(candidate['targets'])) {
                        ...
                        for (const [targetIndex, target] of candidate['targets'].entries()) {
                            if (isRecord(target)) {
                                rejectUnknownProperties(target, ['container', 'coord'], `${label} target ${targetIndex}`, problems);
                            }
                            try {
                                locationKey(target);
                            }
                ...
                if (isRecord(reveal)) {
                    rejectUnknownProperties(reveal, ['commitmentId', 'salt', 'payload'], `${label} reveal`, problems);
                }
            ...
            for (const [index, record] of records.entries()) {
                ...
                for (const field of ['hostTime', 'clientTime']) {
                    if (record[field] !== undefined && !validNonNegativeInteger(record[field])) {
                        problems.push(`record ${index} ${field} must be a non-negative safe integer`);
                    }
                }
                if (![
                    'action',
                ...
                if (kind !== 'action') {
                    rejectUnknownProperties(record, allowedByKind[kind], `record ${index}`, problems);
                }
                if (kind === 'action') {
                        ...
                        if (record['systemInput'] === undefined) {
                            ...
                            if (Array.isArray(record['inputs'])) {
                                try {
                                    ...
                                    if (!record['inputs'].some((input) => canonicalJson(input) === canonicalSystemInput)) {
                    ...
                    if (hasV12IntegritySemantics(header['formatVersion'])) {
                        ...
                        if (timeoutPolicy === undefined) {
                            if (record['timeoutPolicyRef'] !== undefined) {
                                problems.push(`timeout ${index} timeoutPolicyRef requires header.timeoutPolicy`);
                            }
                        }
                        else if (record['timeoutPolicyRef'] !== GAOS_TIMEOUT_POLICY_REF) {
                            problems.push(`timeout ${index} timeoutPolicyRef must be ${GAOS_TIMEOUT_POLICY_REF}`);
                        }
                            ...
                            problems.push(`timeout ${index} tick must equal windowRef + windowTicks`);
                        }
                    }
                }
                    ...
                    if (!hasV12IntegritySemantics(header['formatVersion'])) {
                        problems.push(`interest ${index} requires formatVersion `
                            + `${GAOS_REPLAY_SIGNED_FORMAT_VERSION} or ${GAOS_REPLAY_FORMAT_VERSION}`);
                    }
                    ...
                    for (const field of ['participantId', 'submissionId', 'scopeId']) {
                    ...
                    if (isRecord(attempt)) {
                        rejectUnknownProperties(attempt, ['salt', 'payload'], `commit-mismatch ${index} attemptedReveal`, problems);
                    }
                    ...
                    if (!validNonNegativeInteger(record['tick'])) {
                        problems.push(`seat-signature ${index} tick must be a non-negative safe integer`);
                    }
                    if (typeof record['participantId'] !== 'string'
                        || record['participantId'].length === 0) {
                        problems.push(`seat-signature ${index} participantId must be a non-empty string`);
                    }
                    if (hasV12IntegritySemantics(header['formatVersion'])) {
                        if (!validNonNegativeInteger(record['clientTime'])) {
                            problems.push(`seat-signature ${index} clientTime must be a non-negative safe integer`);
                        }
                        try {
                            signatureBytesFromBase64(record['prevChainHash'], `seat-signature ${index} prevChainHash`, 32);
                        }
                        catch (error) {
                            problems.push(error instanceof Error ? error.message : `seat-signature ${index} chain is invalid`);
                        }
                        try {
                            signatureBytesFromBase64(record['sig'], `seat-signature ${index} sig`, 64);
                        }
                        catch (error) {
                            problems.push(error instanceof Error ? error.message : `seat-signature ${index} sig is invalid`);
                        }
                    }
                }
            }
...
export function serializeReplayJsonl(artifact) {
    const problems = validateReplayArtifact(artifact);
    if (problems.length > 0)
        throw new ReplayFormatError(problems);
...
export function parseReplayJsonl(jsonl) {
...
function signatureSubmissions(artifact) {
    const result = [];
    ...
    for (const [recordIndex, record] of records.entries()) {
        if (record.kind === 'resolution') {
            for (const [inputIndex, input] of record.inputs.entries()) {
                if (typeof input.seat !== 'string' || input.submissionId === undefined)
                    continue;
                result.push({
                    kind: 'submission',
                    submission: {
 ...
 */
export function recheckReplaySignatures(artifact) {
    ...
    let rosterHash;
    try {
        rosterHash = submissionRosterHashV1(artifact.header.seatKeys);
    }
    catch (error) {
        return {
    ...
    const keys = new Map(artifact.header.seatKeys.map((entry) => [entry.id, entry]));
    ...
    for (const entry of keys.values()) {
        checks.set(entry.id, {
            ...
            chainHead: submissionGenesisHashV1(artifact.header.sessionId, entry.id, rosterHash),
            submissionPositions: [],
    ...
    for (const [order, item] of signatureSubmissions(artifact).entries()) {
        if (item.kind === 'periodic') {
            ...
            const valid = verifyEd25519Base64(key.publicKey, periodicSignaturePreimageV1({
        ...
        try {
            ...
            if (canonicalJson(command) !== canonicalCommand)
        ...
        if (value.sig === undefined) {
            if (submission.requiredTier !== undefined) {
                check.policySatisfied = false;
                problems.push(`${submission.label} requires a tier-${submission.requiredTier} signature`);
            }
            continue;
        }
    ...
    for (const [seat, check] of checks) {
        ...
        if (check.firstSubmissionTick !== undefined) {
            for (const submission of check.submissionPositions) {
                const covering = check.attestationPositions.find((candidate) => candidate.order >= submission.order);
                if (covering === undefined) {
                    check.policySatisfied = false;
                    problems.push(`seat ${seat} has no signed chain head covering submission at `
                        + `level ${submission.levelIndex} tick ${submission.tick}`);
                    break;
                }
                if (covering.levelIndex !== submission.levelIndex) {
                    check.policySatisfied = false;
                    problems.push(`seat ${seat} crosses a level boundary without signing its chain head`);
                    break;
                }
                if (covering.tick - submission.tick > key.signingTier.N) {
                    check.policySatisfied = false;
                    problems.push(`seat ${seat} exceeds signingTier.N=${key.signingTier.N} for submission at `
                        + `level ${submission.levelIndex} tick ${submission.tick}`);
                    break;
                }
            }
    ...
    const seats = [...checks.values()].map((check) => ({
        seat: check.seat,
    ...
    const complete = seats.every((seat) => seat.chainReproduced && seat.policySatisfied);
    return {
...
function replaySubmittedAction(input) {
    return {
...
function replayAuthoredAction(input) {
...
function parseCanonicalCommand(command) {
...
function recheckReplaySemantics(artifact, options, contexts) {
    ...
    const adapterFor = (levelIndex) => {
        if (!adapters.has(levelIndex)) {
            const context = contexts.get(levelIndex);
            adapters.set(levelIndex, context === undefined ? undefined : options.semanticAdapterForLevel?.(context));
        }
        return adapters.get(levelIndex);
    };
    const checkSubmission = (levelIndex, tick, input, label) => {
        ...
        try {
            ...
            problems.push(`${label} command mapping failed: `
    ...
    for (const [recordIndex, record] of records.entries()) {
        if (record.kind === 'action') {
            ...
            for (const [inputIndex, input] of record.inputs.entries()) {
                checkSubmission(record.levelIndex, record.tick, input, `resolution ${record.n} input ${inputIndex}`);
            }
        }
        else if (record.kind === 'commit-mismatch'
            ...
            if (base === undefined || mapper === undefined) {
                ...
                try {
                    ...
                    if (canonicalJson(mapped.reveal)
                        !== canonicalJson(expectedReveal)) {
                        problems.push(`commit-mismatch record ${record.n} command maps to a different reveal`);
                    }
                }
                catch (error) {
                    problems.push(`commit-mismatch record ${record.n} command mapping failed: `
                        + `${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
        else if (record.kind === 'timeout') {
            const resolution = records.slice(recordIndex + 1).find((candidate) => candidate.levelIndex === record.levelIndex
            ...
            if (resolution?.kind !== 'resolution'
                ...
                || resolution.systemInput === undefined)
                continue;
            timeoutCount++;
            const base = contexts.get(record.levelIndex);
            ...
            try {
                ...
                problems.push(`timeout ${record.n} mapping failed: `
                    + `${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    const failedSubmissions = problems.some((problem) => problem.includes('command map') || problem.includes('command mapping'));
    const failedTimeouts = problems.some((problem) => problem.startsWith('timeout ') && problem.includes('map'));
    return {
...
function recheckGroupedLevel(artifact, level, reducer, seenSalts) {
    const problems = [];
    const diagnostics = [];
    const commitments = new Map();
    const nextCommitmentId = new Map();
    const seenMismatchSubmissionIds = new Set();
    let lastResolutionTick;
    let state = reducer.init(level.level, level.seed);
    const records = (artifact.records ?? []).filter((record) => record.levelIndex === level.index);
    const noteSalt = (salt, identity, location) => {
        const prior = seenSalts.get(salt);
        if (prior && prior.identity !== identity) {
            diagnostics.push(`salt reuse: ${location} reuses salt first observed at ${prior.location}`);
        }
        else {
    ...
    for (const record of records) {
        const resolution = record.kind === 'resolution'
            ? record
            : record.kind === 'action'
                ? {
                    kind: 'resolution',
        ...
        if (resolution) {
            ...
            for (const replayInput of resolution.inputs) {
                ...
                if (replayInput.commit) {
                    ...
                    if (replayInput.commit.commitmentId !== expectedId) {
                        problems.push(`resolution ${resolution.n} seat ${seat} commitmentId `
                ...
                if (replayInput.reveal) {
                    noteSalt(replayInput.reveal.salt, `${level.index}\u0000${seat ?? '<missing>'}\u0000${replayInput.reveal.commitmentId}`, `resolution ${resolution.n} seat ${seat ?? '<missing>'}`);
                    if (!seat) {
                    ...
                    if (!commitment) {
                        problems.push(`resolution ${resolution.n} reveal references unknown commitment `
                            + `${seat}/${replayInput.reveal.commitmentId}`);
                        continue;
                    }
                    if (commitment.revealed) {
                        problems.push(`resolution ${resolution.n} commitment ${seat}/${replayInput.reveal.commitmentId}`
                    ...
                    try {
                        ...
                        }, replayInput.reveal.salt, replayInput.reveal.payload);
                    }
                    catch (error) {
                        problems.push(`resolution ${resolution.n} reveal is malformed: `
                            + `${error instanceof Error ? error.message : String(error)}`);
                        continue;
                    }
                    if (actualHash !== commitment.envelope.hash) {
                        problems.push(`commit_mismatch: seat ${seat}, commitmentId ${replayInput.reveal.commitmentId}`);
                        continue;
                    }
            ...
            try {
                ...
                problems.push(`resolution ${resolution.n} rejected on replay: `
                    + `${error instanceof Error ? error.message : String(error)}`);
                break;
            }
        }
            ...
            if (seenMismatchSubmissionIds.has(submissionKey)) {
                problems.push(`commit-mismatch record ${record.n} reuses submissionId `
            ...
            if (expectedTick !== undefined && record.tick !== expectedTick) {
                problems.push(`commit-mismatch record ${record.n} tick ${record.tick} must match `
                    + `the open tick ${expectedTick}`);
            }
            const commitment = commitments.get(key);
            if (!commitment) {
                problems.push(`commit-mismatch record ${record.n} references unknown commitment `
                    + `${record.participantId}/${record.commitmentId}`);
            }
            else if (commitment.revealed) {
                problems.push(`commit-mismatch record ${record.n} references already revealed commitment `
                    + `${record.participantId}/${record.commitmentId}`);
            }
            else if (duplicate) {
                continue;
                ...
                diagnostics.push(`commit-mismatch record ${record.n} was recorded but is not independently recheckable`);
            }
            else {
                noteSalt(record.attemptedReveal.salt, `${level.index}\u0000${record.participantId}\u0000${record.commitmentId}`, `commit-mismatch record ${record.n} seat ${record.participantId}`);
                try {
                    const actualHash = createCommitmentHash({
                        ...
                        commitmentId: commitment.envelope.commitmentId,
                        windowRef: commitment.windowRef,
                    }, record.attemptedReveal.salt, record.attemptedReveal.payload);
                    if (actualHash === commitment.envelope.hash) {
                        problems.push(`commit-mismatch record ${record.n} is inconsistent: attempted reveal matches`);
                    }
                    else {
                        diagnostics.push(`verified commit_mismatch: seat ${record.participantId}, `
                            + `commitmentId ${record.commitmentId}`);
                    }
                }
                catch (error) {
                    problems.push(`commit-mismatch record ${record.n} cannot be rechecked: `
                        + `${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
    ...
    if (actionsUsed >= 0 && actionsUsed !== level.result.actionsUsed) {
        problems.push(`actionsUsed: recorded ${level.result.actionsUsed}, replayed ${actionsUsed}`);
    }
    return {
 ...
 */
export function recheckReplayArtifact(artifact, resolveReducer, options = {}) {
    const problems = validateReplayArtifact(artifact);
    const diagnostics = [];
    if (problems.length > 0) {
        return {
    ...
    const signatures = recheckReplaySignatures(artifact);
    const checks = [];
    let totalStars = 0;
    ...
    if (dmathDeclaration !== undefined) {
        try {
            ...
            problems.push(`cannot construct replay dmath algorithm: `
                + `${error instanceof Error ? error.message : String(error)}`);
            return {
                ok: false,
                verdict: 'rejected',
    ...
    const seenSalts = new Map();
    const replayContexts = new Map();
    for (const level of artifact.header.levels) {
        ...
        if (!reducer) {
            problems.push(`level ${level.index} (${level.id}): no reducer for `
        ...
        const result = artifact.records
            ...
            }, actions, options.optionsForLevel?.(context));
        checks.push({ index: level.index, id: level.id, seed: level.seed, result });
        problems.push(...result.problems.map((problem) => `level ${level.index} (${level.id}): ${problem}`));
        diagnostics.push(...result.diagnostics.map((diagnostic) => `level ${level.index} (${level.id}): ${diagnostic}`));
    ...
    if (checks.length === artifact.header.levels.length) {
        if (totalStars !== artifact.header.totals.totalStars) {
            problems.push(`totalStars: recorded ${artifact.header.totals.totalStars}, replayed ${totalStars}`);
        }
        if (totalActionsUsed !== artifact.header.totals.totalActionsUsed) {
            problems.push(`totalActionsUsed: recorded ${artifact.header.totals.totalActionsUsed}, `
    ...
    if (semantics.submissions === 'unavailable') {
        diagnostics.push('signed command bytes were authenticated, but command-to-action mapping was not supplied');
    }
    if (semantics.timeouts === 'unavailable') {
        diagnostics.push('timeout actions were replayed, but timeoutToAction mapping was not supplied');
    }
