...
export function shortestGridPath(options) {
    ...
    while (cursor < queue.length) {
        ...
        for (const [dx, dy] of steps) {
            ...
            if (isBlocked(next) && !(isGoal && options.allowBlockedGoal))
                continue;
 ...
 */
export function nearestReachableCellPath(options) {
        ...
        for (let cursor = layerStart; cursor < layerEnd; cursor++) {
            ...
            for (const [dx, dy] of steps) {
                ...
                if (next[0] < 0 || next[1] < 0 || next[0] >= width || next[1] >= height)
                    continue;
                const nextKey = key(next);
                if (previous.has(nextKey) || isBlocked(next))
                    continue;
                previous.set(nextKey, current);
                queue.push(next);
            }
...
export function lineOfSightClear(from, to, isBlocked) {
...
export function coneFieldCells(options) {
    ...
    for (let forward = 1; forward <= options.range; forward++) {
        for (let perpendicular = -(forward - 1); perpendicular <= forward - 1; perpendicular++) {
            ...
            if (!options.cellExists(cell) || options.isBlocked(cell))
                continue;
            if (lineOfSightClear(options.from, cell, options.isBlocked))
                cells.push(cell);
        }
    }
    return cells;
