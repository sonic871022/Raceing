...
function utf8(value, label, allowEmpty = false) {
...
function framedSize(bytes) {
    return 4 + bytes.length;
}
function writeFramed(target, offset, bytes) {
...
export function signatureBytesToBase64(bytes) {
    let binary = '';
...
export function signatureBytesFromBase64(value, label, expectedLength) {
    if (typeof value !== 'string'
...
export function canonicalSubmissionCommandV1(command) {
...
export function submissionPreimageV1(envelope) {
    ...
    const previous = signatureBytesFromBase64(envelope.prevChainHash, 'prevChainHash', 32);
    const result = new Uint8Array(framedSize(DOMAIN_TAG)
        + framedSize(session)
        + framedSize(seat)
...
export function submissionChainHashV1(envelope) {
...
export function submissionRosterHashV1(seatKeys) {
    ...
    const normalized = seatKeys.map((entry, index) => {
        ...
        if (ids.has(entry.id))
            throw new TypeError(`seatKeys contains duplicate id ${entry.id}`);
        ids.add(entry.id);
        ...
        const leftPoints = Array.from(left.id, (value) => value.codePointAt(0));
        const rightPoints = Array.from(right.id, (value) => value.codePointAt(0));
...
export function submissionGenesisHashV1(sessionId, seat, rosterHash) {
 ...
 */
export function periodicSignaturePreimageV1(envelope) {
    assertU64(envelope.tick, 'tick');
    assertU64(envelope.clientTime, 'clientTime');
    const session = utf8(envelope.sessionId, 'sessionId');
    const seat = utf8(envelope.seat, 'seat');
    const chainHead = signatureBytesFromBase64(envelope.chainHead, 'chainHead', 32);
    const result = new Uint8Array(framedSize(PERIODIC_DOMAIN_TAG)
        + framedSize(session)
        + framedSize(seat)
        + 8
        + 8
...
function decodePoint(bytes) {
    ...
    if (mod(v * x * x - u) !== 0n)
        x = mod(x * SQRT_M1);
    if (mod(v * x * x - u) !== 0n || (x === 0n && sign === 1))
        return undefined;
    if (Number(x & 1n) !== sign)
        x = P - x;
    return [x, y, 1n, mod(x * y)];
}
const BASE_ENCODING = Uint8Array.from([0x58, ...new Array(31).fill(0x66)]);
...
function windowTable(point) {
    const table = [IDENTITY, point];
...
function doubleScalarBaseMinusPoint(baseScalar, point, pointScalar) {
    ...
    for (let window = 63; window >= 0; window--) {
        ...
        if (baseDigit !== 0)
            result = pointAdd(result, BASE_WINDOW[baseDigit]);
        if (pointDigit !== 0)
            result = pointAdd(result, pointWindow[pointDigit]);
    }
    return result;
}
const PUBLIC_POINT_CACHE_LIMIT = 64;
const publicPointCache = new Map();
function decodedPublicPoint(publicKey) {
    ...
    if (point === undefined || hasSmallOrder(point))
        return undefined;
    if (publicPointCache.size >= PUBLIC_POINT_CACHE_LIMIT) {
        const oldest = publicPointCache.keys().next().value;
        if (oldest !== undefined)
            publicPointCache.delete(oldest);
    }
...
export function verifyEd25519(publicKey, message, signature) {
    ...
    const challenge = littleEndianInteger(sha512(concatBytes(rBytes, publicKey, message))) % L;
    const reconstructed = doubleScalarBaseMinusPoint(s, publicPoint, challenge);
    return pointsEqual(pointDouble(pointDouble(pointDouble(reconstructed))), pointDouble(pointDouble(pointDouble(rPoint))));
}
/** Verify canonical base64 Ed25519 material without throwing. */
export function verifyEd25519Base64(publicKey, message, signature) {
    try {
        return verifyEd25519(signatureBytesFromBase64(publicKey, 'publicKey', 32), message, signatureBytesFromBase64(signature, 'sig', 64));
    }
    catch {
        return false;
    }
...
export async function generateSubmissionKeyPair() {
...
export async function exportSubmissionPublicKey(publicKey) {
    const raw = new Uint8Array(await crypto.subtle.exportKey('raw', publicKey));
    if (raw.length !== 32)
        throw new TypeError('Ed25519 public key must contain 32 raw bytes');
    return signatureBytesToBase64(raw);
}
/** Sign arbitrary bytes with a WebCrypto Ed25519 private key. */
export async function signEd25519Base64(privateKey, message) {
    const messageBuffer = message.slice().buffer;
    const signature = new Uint8Array(await crypto.subtle.sign(SUBMISSION_SIGNATURE_ALGORITHM, privateKey, messageBuffer));
...
export function signSubmissionV1(privateKey, envelope) {
    return signEd25519Base64(privateKey, submissionPreimageV1(envelope));
}
/** Sign a periodic RFC-010 chain-head checkpoint. */
export function signPeriodicChainHeadV1(privateKey, envelope) {
