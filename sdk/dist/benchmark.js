...
export function assertBenchmarkManifest(manifest) {
    ...
    if (manifest.observationModalities !== undefined) {
        if (!Array.isArray(manifest.observationModalities)
            || manifest.observationModalities.some((modality) => typeof modality !== 'string' || modality.length === 0)) {
    ...
    if (manifest.authorityRequirements !== undefined) {
        ...
        for (const requirement of manifest.authorityRequirements) {
            ...
            if (claims.has(identity))
                throw new TypeError(`duplicate authority requirement ${identity}`);
            claims.add(identity);
        }
    }
    if (manifest.tasks.length === 0)
        throw new TypeError('manifest must contain tasks');
    const ids = new Set();
    for (const task of manifest.tasks) {
        assertNonEmpty(task.id, 'task.id');
        if (ids.has(task.id))
            throw new TypeError(`duplicate benchmark task ${task.id}`);
        ids.add(task.id);
...
export function benchmarkManifestDigest(manifest) {
    assertBenchmarkManifest(manifest);
    return bytesToHex(sha256(encoder.encode(canonicalJson(manifest))));
}
function episodeId(plan) {
    return `${plan.taskId}-seed-${plan.seed}-episode-${plan.episode}`;
}
function assertEpisodeResult(expected, result) {
    if (canonicalJson(result.plan)
...
function aggregateEpisodeResults(manifest, completed) {
    ...
    return aggregateBenchmarkScores(manifest, manifest.tasks.map((task) => {
 ...
 */
export async function runBenchmark(manifest, adapter, options = {}) {
    ...
    if (options.resume !== undefined) {
        if (options.resume.schema !== 'gaos.benchmark-run-checkpoint.v1'
            ...
            || canonicalJson(options.resume.plan)
                !== canonicalJson(plan)
    ...
    let cursor = 0;
    const workers = Array.from({ length: Math.min(parallelism, limit) }, async () => {
        while (cursor < limit) {
    ...
    const ordered = [...completed.values()].sort((left, right) => left.plan.index - right.plan.index);
    const checkpoint = {
    ...
    if (ordered.length !== plan.length)
        return { status: 'interrupted', checkpoint };
    return {
        status: 'complete',
        checkpoint,
...
export function packBenchmarkRun(manifest, run, submission) {
    ...
    const episodes = [...run.checkpoint.completed]
        ...
        const replayDigest = bytesToHex(sha256(encoder.encode(canonicalJson(replay))));
        return {
    ...
    const contentDigest = bytesToHex(sha256(encoder.encode(canonicalJson(content))));
    if (submission.attestations?.some((attestation) => attestation.subjectDigest !== contentDigest)) {
...
export function benchmarkBundleContentDigest(bundle) {
    const content = {
    ...
    return bytesToHex(sha256(encoder.encode(canonicalJson(content))));
}
/** Attach receipts only after their subject is the already-computed content digest. */
export function attachBenchmarkAttestations(bundle, attestations) {
...
export function emptyVerificationFacts() {
    return {
        replay: 'not-observed',
...
export function benchmarkBundleFiles(bundle, verification = emptyVerificationFacts()) {
    const files = {
        'manifest.json': `${canonicalJson(bundle.manifest)}\n`,
        ...
        'verification.json': `${canonicalJson(verification)}\n`,
        'README.md': [
...
export function benchmarkPackageDigest(files) {
...
function combinedState(values, notRequired) {
    if (notRequired)
        return 'not-required';
...
function authorityFact(claim) {
    if (claim === 'identity')
        return 'accountIdentityAttested';
    if (claim === 'time')
        return 'timeAttested';
    if (claim === 'publication')
        return 'publicationLogged';
    if (claim === 'tail-anchor')
        return 'tailAnchored';
    if (claim === 'model-identity')
        return 'modelIdentityAttested';
 ...
 */
export async function verifyBenchmarkBundle(bundle, manifest, verifyEpisode, options = {}) {
    ...
    for (const [bundleIndex, episode] of bundle.episodes.entries()) {
        const localReasons = [];
        if (!expectedIds.has(episode.id))
            localReasons.push('episode is not required by manifest');
        if (seen.has(episode.id))
            localReasons.push('duplicate episode');
        seen.add(episode.id);
        const expectedPlan = plan[bundleIndex];
        if (expectedPlan === undefined
            || episode.plan.index !== bundleIndex
            || canonicalJson(expectedPlan)
                !== canonicalJson(episode.plan)) {
            localReasons.push('episode plan does not exactly match the authored plan');
        }
        if (episode.id !== episodeId(episode.plan))
            localReasons.push('episode id does not match plan');
        const replayDigest = bytesToHex(sha256(encoder.encode(canonicalJson(episode.replay))));
        if (episode.replayDigest !== replayDigest)
            localReasons.push('replay digest mismatch');
        let checked;
        try {
        ...
        if (!checked.replayValid)
            localReasons.push('episode replay verification failed');
        if (checked.score !== episode.score)
            localReasons.push('carried episode score is incorrect');
    ...
    for (const id of expectedIds) {
        if (!seen.has(id))
            reasons.push(`missing required episode ${id}`);
    }
    if (seen.size !== expectedIds.size)
        reasons.push('bundle episode set is incompatible');
    reasons.push(...episodeFacts.flatMap((fact) => fact.reasons.map((reason) => `${fact.id}: ${reason}`)));
    let aggregate;
    if (recomputed.length === plan.length && reasons.length === 0) {
        aggregate = aggregateEpisodeResults(manifest, recomputed);
        if (canonicalJson(aggregate)
            !== canonicalJson(bundle.scores)) {
            reasons.push('carried task or aggregate scores are incorrect');
        }
    }
    for (const requirement of manifest.authorityRequirements ?? []) {
        const field = authorityFact(requirement.claim);
        const candidates = (bundle.submission.attestations ?? []).filter((attestation) => attestation.authority.authorityId === requirement.authorityId
        ...
        if (options.externalTrustResolver !== undefined) {
            for (const attestation of candidates) {
                results.push(await verifyExternalAttestation(attestation, contentDigest, {
        ...
        if (requirement.required && !accepted) {
            reasons.push(`required ${requirement.claim} attestation from ${requirement.authorityId} is missing or invalid`);
        }
    }
    facts.replay = episodeFacts.every((fact) => fact.replayValid) ? 'verified' : 'failed';
    facts.signatures = combinedState(episodeFacts.map((fact) => fact.signatures), !manifest.submission.requireSignedSeats);
    facts.semantics = combinedState(episodeFacts.map((fact) => fact.semantics), false);
    facts.evidenceComplete = combinedState(episodeFacts.map((fact) => fact.evidenceComplete), !manifest.submission.requireCompleteCoverage);
...
export function planBenchmarkEpisodes(manifest) {
    assertBenchmarkManifest(manifest);
    const plan = [];
    for (const task of manifest.tasks) {
...
export function aggregateBenchmarkScores(manifest, scores) {
    assertBenchmarkManifest(manifest);
    const byTask = new Map();
    for (const { taskId, score } of scores) {
        if (byTask.has(taskId))
            throw new TypeError(`duplicate score for task ${taskId}`);
        if (!Number.isFinite(score))
            throw new RangeError(`score for task ${taskId} must be finite`);
    ...
    for (const task of manifest.tasks) {
        ...
        if (score === undefined)
            throw new TypeError(`missing score for task ${task.id}`);
    ...
    if (totalWeight === 0)
        throw new RangeError('aggregate task weight must be greater than zero');
