...
import { packBenchmarkRun, benchmarkBundleFiles, runBenchmark, verifyBenchmarkBundle, } from './benchmark.js';
function usage() {
    return [
        'usage:',
...
async function loadAgent(cwd, path) {
    if (!path)
        throw new TypeError('--agent requires a module path');
    const loaded = await import(pathToFileURL(resolve(cwd, path)).href);
...
async function writePackageDirectory(path, files) {
...
async function readPackageDirectory(path) {
    ...
    for (const required of ['manifest.json', 'submission.json', 'scores.json', 'verification.json', 'README.md']) {
        if (!actual.has(required))
            throw new TypeError(`package is missing ${required}`);
    }
    const manifest = JSON.parse(await readFile(resolve(path, 'manifest.json'), 'utf8'));
    const submissionFile = JSON.parse(await readFile(resolve(path, 'submission.json'), 'utf8'));
    const scoreFile = JSON.parse(await readFile(resolve(path, 'scores.json'), 'utf8'));
    const episodes = [];
    ...
    for (const metadata of scoreFile.episodes) {
        ...
        if (expected.has(relative))
            throw new TypeError(`duplicate episode file ${relative}`);
        expected.add(relative);
        const replayText = await readFile(resolve(path, relative), 'utf8');
        const replay = metadata.replayEncoding === 'json'
    ...
    const { schema, contentDigest, manifestDigest, ...submission } = submissionFile;
    const bundle = {
        schema,
        contentDigest,
        manifest,
...
async function enumeratePackageFiles(root) {
    ...
    async function visit(directory, prefix) {
        for (const entry of await readdir(directory, { withFileTypes: true })) {
            ...
            if (stat.isSymbolicLink())
                throw new TypeError(`package contains symlink ${relative}`);
            if (stat.isDirectory())
                await visit(absolute, relative);
            else if (stat.isFile()) {
                if (result.has(relative))
                    throw new TypeError(`duplicate package path ${relative}`);
                result.add(relative);
            }
            else
                throw new TypeError(`unsupported package entry ${relative}`);
        }
    }
    await visit(root, '');
    return result;
...
export async function runBenchmarkCli(argv, io = {}) {
    ...
    try {
        ...
        if (command === 'run') {
            if (!argv[2])
                throw new TypeError('run requires a manifest path');
            const manifest = JSON.parse(await readFile(resolve(cwd, argv[2]), 'utf8'));
            ...
            const value = { schema: 'gaos.benchmark-run-file.v1', manifest, run };
        ...
        if (command === 'resume') {
            if (!argv[2])
                throw new TypeError('resume requires a run directory');
            const directory = resolve(cwd, argv[2]);
            const saved = JSON.parse(await readFile(resolve(directory, 'run.json'), 'utf8'));
            const adapter = await loadAgent(cwd, option(argv, 'agent'));
            ...
            await writeFile(resolve(directory, 'run.json'), `${JSON.stringify({ ...saved, run }, null, 2)}\n`);
            stdout(`${directory}\n`);
            return run.status === 'complete' ? 0 : 1;
        }
        if (command === 'pack') {
            if (!argv[2])
                throw new TypeError('pack requires a run directory');
            const saved = JSON.parse(await readFile(resolve(cwd, argv[2], 'run.json'), 'utf8'));
        ...
        if (command === 'verify') {
            if (!argv[2])
                throw new TypeError('verify requires a bundle path');
            ...
            const manifest = JSON.parse(await readFile(resolve(cwd, manifestPath), 'utf8'));
            const loaded = await import(pathToFileURL(resolve(cwd, adapterPath)).href);
