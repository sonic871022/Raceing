...
import { createDefaultKeyedProviderRegistry, createKeyedAgentDriver, runAgentDriverEpisode, } from '../agent/index.js';
...
async function check(args, io) {
    ...
    const id = args.positionals[0];
    if (!id)
        throw new TypeError('check requires a provider id');
    ...
    if (!result.ok)
        io.stderr(`${provider.login}\n`);
    return result.ok ? 0 : 1;
}
async function spawnAgent(args, io) {
    assertOptions(args.options, ['mcp-url', 'prompt', 'server-name', 'tools']);
    const id = args.positionals[0];
    if (!id)
        throw new TypeError('spawn requires a CLI agent id');
    ...
    try {
        ...
        if (result.code && result.stderrTail)
...
async function runKeyed(args, io) {
    ...
    const driver = createKeyedAgentDriver(provider.id, {
        apiKey,
...
export async function runAgentCli(argv, io = defaultIo()) {
    ...
    if (command === 'drivers')
        return listDrivers(io);
    if (command === 'status')
        return status(args, io);
    if (command === 'check')
        return check(args, io);
    if (command === 'spawn')
        return spawnAgent(args, io);
    if (command === 'run')
        return runKeyed(args, io);
