...
export class CliAgentRegistry {
    ...
    register(agent, options = {}) {
        if (!agent.id.trim())
            throw new TypeError('CLI agent id must not be empty');
...
export function parseCodexJson(line) {
    let event;
    try {
        event = JSON.parse(line);
    }
    ...
    if (kind !== 'agent_message' && kind !== 'agent_reasoning' && kind !== 'reasoning')
        return [];
    const value = item.text ?? item.message;
    return typeof value === 'string' && value.trim() ? [value.trim()] : [];
}
export function parseGenericLine(line) {
...
function claudeLaunchArgs(context, options = {}) {
...
export function customCliAgentsFromJson(raw) {
    ...
    for (const [id, value] of Object.entries(parsed)) {
        ...
        agents.push({
            ...
            launch: (context) => ({
                ...
                files: Object.fromEntries(Object.entries(definition.files ?? {}).map(([path, content]) => [path, fill(content, context)])),
            }),
            parseLine: parseGenericLine,
...
export function createDefaultCliAgentRegistry(options = {}) {
    ...
    if (options.customAgentsJson) {
        for (const agent of customCliAgentsFromJson(options.customAgentsJson))
