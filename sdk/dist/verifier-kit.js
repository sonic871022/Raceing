...
function normalizeKitPath(value) {
    if (value.length === 0
        || value.includes('\\')
        || value.startsWith('/')
        || value.includes('\0')
...
function writeOctal(target, offset, length, value) {
    ...
    if (encoded.length >= length)
        throw new TypeError('tar numeric field overflow');
...
function readOctal(bytes, offset, length, label) {
    const value = readString(bytes, offset, length).trim();
    if (!/^[0-7]+$/.test(value))
        throw new TypeError(`malformed tar ${label}`);
    const parsed = Number.parseInt(value, 8);
    if (!Number.isSafeInteger(parsed))
        throw new TypeError(`tar ${label} exceeds safe range`);
    return parsed;
}
...
function decodeTar(bytes, limits = {}) {
    const maxBytes = limits.maxBytes ?? DEFAULT_MAX_BYTES;
    const maxFiles = limits.maxFiles ?? DEFAULT_MAX_FILES;
    if (bytes.length > maxBytes)
        throw new TypeError('verifier kit exceeds size limit');
    ...
    while (!isZeroBlock(bytes, offset)) {
        if (entries.length >= maxFiles)
            throw new TypeError('verifier kit exceeds file limit');
        ...
        if (paths.has(path))
            throw new TypeError(`duplicate verifier-kit path ${path}`);
        paths.add(path);
    ...
    if (offset + BLOCK_SIZE * 2 !== bytes.length
...
export function assertVerifierKitManifest(value) {
    ...
    if (!game || Array.isArray(game))
        throw new TypeError('manifest.game must be an object');
    if (!adapter || Array.isArray(adapter))
        throw new TypeError('manifest.adapter must be an object');
    ...
    if (!Array.isArray(manifest['replayFormats'])
...
export function assertVerifierReference(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    ...
    if (!Array.isArray(reference['mirrors'])
...
export function verifierReferenceFromExtensions(extensions) {
...
export function packVerifierKit(input) {
...
export function inspectVerifierKit(input, limits = {}) {
    ...
    for (const expected of manifest.files) {
        ...
        if (!actual)
            throw new TypeError(`verifier kit is missing ${expected.path}`);
...
function cachePath(cacheDirectory, digest) {
...
export async function admitVerifierKit(cacheDirectory, bytes, expectedDigest, limits = {}) {
...
export async function readCachedVerifierKit(cacheDirectory, digest, limits = {}) {
    let bytes;
    try {
...
export async function resolveVerifierKit(options) {
    ...
    try {
        const cached = await readCachedVerifierKit(options.cacheDirectory, reference.digest, options.limits);
        if (cached) {
            return {
    ...
    for (const mirror of reference.mirrors) {
        try {
            ...
            await admitVerifierKit(options.cacheDirectory, bytes, reference.digest, options.limits);
            return {
                resolution: {
                    ...base,
                    retrieval: 'fetched',
            ...
            diagnostics.push(`mirror ${mirror} failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return {
        resolution: {
...
export async function extractVerifierKit(kit, destination) {
    ...
    await writeFile(join(root, 'verifier-manifest.json'), `${canonicalJson(kit.manifest)}\n`, { flag: 'wx', mode: 0o444 });
}
export async function runRestrictedVerifier(runner, request, limits = {}) {
    if (!runner) {
        return {
            resolution: {
    ...
    if (response.schema !== 'gaos.verifier-response.v1'
