...
function decodePointer(path) {
    ...
    return path.slice(1).split('/').map((token) => {
        if (/~(?![01])/u.test(token))
            throw new TypeError('JSON Pointer contains invalid escape');
...
function diffJson(previous, next, options = {}) {
    const operations = [];
    ...
    const emit = (operation) => {
        ...
        if (options.canonical || options.maxBytes !== undefined) {
            const encoded = canonicalJson(operation);
...
export function createJsonPatch(previous, next) {
 ...
 */
export function createValidatedJsonPatch(previous, next, maxOperations) {
 ...
 * @internal Inputs have already passed the session canonical JSON boundary.
 */
export function createBoundedValidatedJsonPatch(previous, next, maxOperations, maxBytes) {
...
export function applyJsonPatch(previous, operations) {
    assertJsonValue(previous, 'previous view');
    ...
    for (const operation of operations) {
        ...
        if (tokens.length === 0) {
            if (operation.op === 'remove')
                throw new TypeError('root removal is not supported');
        ...
        if (!objectValue(parent))
            throw new TypeError('patch parent must be an object');
        const key = tokens.at(-1);
        if (operation.op === 'remove') {
            if (!Object.hasOwn(parent, key))
                throw new TypeError('remove path does not exist');
            delete parent[key];
        }
        else {
            assertJsonValue(operation.value, 'patch value');
 ...
 */
function jsonProjection(source, projected) {
    if (jsonEqual(source, projected))
        return true;
    if (Array.isArray(source) && Array.isArray(projected)) {
        let sourceIndex = 0;
        for (const item of projected) {
            while (sourceIndex < source.length
                && !jsonProjection(source[sourceIndex], item))
                sourceIndex++;
    ...
    if (objectValue(source) && objectValue(projected)) {
        return Object.keys(projected).every((key) => Object.hasOwn(source, key) && jsonProjection(source[key], projected[key]));
    }
    return false;
}
export function isJsonProjection(source, projected) {
