...
import { admitVerifierKit, inspectVerifierKit, packVerifierKit, VERIFIER_KIT_EXTENSION, } from './verifier-kit.js';
function usage() {
    return [
        'usage:',
...
function assertBundledEntrypoint(source) {
    const text = Buffer.from(source).toString('utf8');
    if (/(?:^|\n)\s*import\s+(?!\()/.test(text)
        || /(?:^|\n)\s*export\s+[^;\n]*\sfrom\s*['"]/.test(text)
        || /\bimport\s*\(/.test(text)
        || /\brequire\s*\(/.test(text)) {
        throw new TypeError('adapter entry point must be self-contained; bundle imports before packing');
    }
}
...
export async function runVerifierKitCli(argv, io = {}) {
    ...
    try {
        if (argv[1] === 'pack') {
            ...
            for (let index = 3; index < argv.length; index++) {
                ...
                if (argument === '--game')
                    gameValue = argv[++index];
                else if (argument === '--adapter')
                    adapterValue = argv[++index];
            ...
            if (!entrypoint)
                throw new TypeError('pack requires an explicit adapter entry point');
            ...
            const outputPath = resolve(cwd, output ?? `${basename(entrypoint, '.mjs')}.${VERIFIER_KIT_EXTENSION}`);
        ...
        if (argv[1] === 'inspect') {
            const path = argv[2];
            if (!path)
                throw new TypeError('inspect requires a verifier-kit path');
        ...
        if (argv[1] === 'fetch') {
            ...
            const path = await admitVerifierKit(resolve(cwd, cache), await boundedResponseBytes(response), digest);
